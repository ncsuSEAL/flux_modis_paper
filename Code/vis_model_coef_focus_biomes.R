#************************************************************************************
# Description: Model coefficients for focused biome types
...
gsl_coef_fig <- ggplot() +
    ...
    geom_pointrange(aes(x = IGBP, ymin = lwr, y = med, ymax = upr, 
        color = modis_color), shape = 16, alpha = 0.5, size = 0.3, 
        data = m_model$beta2[IGBP %in% focus_biomes], 
        position = position_dodge2(width = 0.5)
    ) +
    geom_pointrange(aes(x = IGBP, ymin = lwr, y = med, ymax = upr, 
        fill = modis_color), shape = 21, stroke = 1, size = 0.8, 
        data = m_model$eta2[IGBP %in% focus_biomes], 
        position = position_nudge(x = -0.35)
    ) +
    geom_pointrange(aes(x = IGBP, ymin = lwr, y = med, ymax = upr, 
        color = flux_color), shape = 16, alpha = 0.5, size = 0.3, 
        data = f_model$beta2[IGBP %in% focus_biomes], 
        position = position_dodge2(width = 0.5)
    ) +
    geom_pointrange(aes(x = IGBP, ymin = lwr, y = med, ymax = upr, 
        fill = flux_color), shape = 21, size = 0.8, 
        data = f_model$eta2[IGBP %in% focus_biomes], 
    ...
    geom_text(aes(x = IGBP, y = rep(-15, 6), label = N),
        data = unique(north_sites_dt[, .(site, IGBP)])[
            IGBP %in% focus_biomes, .N, by = c("IGBP")
        ]
    ) +
    theme_article() +
...
# ggsave(filename = "0_Manuscript/Fig_2_xx.png", plot = gsl_coef_fig, 
#    width = 7, height = 4, bg = "white")
ggsave(filename = "Output/model_coef_focus_biomes.png", plot = gsl_coef_fig, 
    width = 7, height = 4, bg = "white")
